...
create-dev-container container-name volume-name python-version extra-mounts="":
  docker run --rm -it --env HOST_USER_HOME="${HOME}" --mount type=bind,source=/var/run/docker.sock,target=/var/run/docker.sock pabuk/dev-python:{{python-version}} /bin/zsh -c "dc create-dev-container {{container-name}} {{volume-name}} {{python-version}} .zsh-extra {{extra-mounts}}"
...
create-dev-container-no-pull container-name volume-name python-version extra-mounts="":
  docker run --rm -it --env HOST_USER_HOME="${HOME}" --mount type=bind,source=/var/run/docker.sock,target=/var/run/docker.sock pabuk/dev-python:{{python-version}} /bin/zsh -c "NO_PULL=1 dc create-dev-container {{container-name}} {{volume-name}} {{python-version}} .zsh-extra {{extra-mounts}}"
